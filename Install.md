...
Возможные проблемы:
- **Не открывается сайт вообще или выводит совсем не то.** Обычно это значит, что Nginx не запустился. Возможно, у вас запущен другой сервер на 80 порту, например Apache.
- **Сайт пишет "It works".** Это значит, что неверно настроен Nginx, проверьте, правильно ли был указан корень `root` при `config:nginx`.
- **Стили не грузятся.** Возможно, у Nginx не хватает прав на чтение из директории сайта. Проверьте, что `+r` есть на всей директории проекта, а `+rx` -- на родительских директориях.
